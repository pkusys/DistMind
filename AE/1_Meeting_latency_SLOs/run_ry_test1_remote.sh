...
python ./source/py_utils/launch_remote.py --launch_part create_dir  --log_dir tmp/test1/ray/$MODEL_SEED --test_index 1
sleep 5

# Function to kill all subprocesses
cleanup() {
    echo "Cleaning up all background processes..."
    python ./source/py_utils/launch_remote.py --launch_part ray_server --test_index 1 --stop
...
# Step2 : run server
echo "Running server..."
python ./source/py_utils/launch_remote.py --launch_part ray_server --test_index 1 --model_seed $MODEL_SEED --n_models $NUM_MODELS
