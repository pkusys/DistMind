...
setsid python source/mps/server_agent.py --lb-port $PORT_FOR_SERVER --lb-ip $IP_FOR_LOAD_BALANCER --size-list build/mps/model_sizes.txt --gpu-num $WORLD_SIZE --ctrl-ip $CONTROLLER_IP --ctrl-port $CONTROLLER_PORT_SERVER --local-ip $LOCAL_IP --training-log-dir "../../$PRIFIX/mps/training_logs" > $PRIFIX/mps/log_server.txt 2>&1 &
PID_SERVER=$!

# Step 8: Wait for readiness
echo "Waiting 400 seconds for running train..."
sleep 400
