...
cleanup() {
    ...
    for pid in $PID_CLIENT_INFERENCE $PID_CACHE $PID_LB $PID_CONTROLLER $PID_STORAGE $PID_METADATA; do
        if kill -0 $pid 2>/dev/null; then
            kill -9 $pid
            sleep 2
        fi
...
./build/bin/storage $STORAGE_PORT $STORAGE_PORT > $PRIFIX/gpu/log_storage.txt 2>&1 &
PID_STORAGE=$!
...
sleep 1
FI_EFA_ENABLE_SHM_TRANSFER=0 ./build/bin/cache $LOCAL_IP $CACHE_PORT $METADATA_IP $METADATA_PORT $LOAD_BALANCER_IP $LOAD_BALANCER_PORT_CACHE cachecache $CACHE_SIZE $CACHE_BLOCK_SIZE > $PRIFIX/gpu/log_cache.txt 2>&1 &
PID_CACHE=$!
...
do
	...
	nohup python ./build/bin/server.py $LOCAL_IP $GPU $LOCAL_IP $CACHE_PORT $LOAD_BALANCER_IP $LOAD_BALANCER_PORT_SERVER $CONTROLLER_IP $CONTROLLER_PORT_SERVER $BACKEND $WORLD_SIZE >$PRIFIX/gpu/log_worker_$GPU.txt 2>&1 &
    PID_SERVER+=($!)
    sleep 1
done
...
setsid python build/bin/client_max_inference.py $CONTROLLER_IP $CONTROLLER_PORT $LOAD_BALANCER_IP $PORT_FOR_CLIENT 0 > $PRIFIX/gpu/log_client.txt 2>&1 &
