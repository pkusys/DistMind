...
public:
    ...
    void push(std::shared_ptr<U> task, bool train = false);
    std::shared_ptr<U> dispatcherPop(int max_idle = 0);
...
private:
    ...
    balance::util::AtomicQueue<std::shared_ptr<U> > _q_in_train;
...
template<class U, class V>
void Dispatcher<U, V>::push(std::shared_ptr<U> task, bool train) {
    const std::lock_guard<std::mutex> guard(_lock_in);
    if (train)
        _q_in_train.push(task);
    else
        _q_in.push(task);
}

template<class U, class V>
std::shared_ptr<U> Dispatcher<U, V>::dispatcherPop(int max_idle) {
    const std::lock_guard<std::mutex> guard(_lock_in);
    
    if (!_q_in.empty())
        return _q_in.pop();
    
    if (max_idle > 0 && !_q_in_train.empty())
        return _q_in_train.pop();
