...
./build/bin/metadata_storage 0.0.0.0 $METADATA_PORT > tmp/distmind/log_metadata.txt 2>&1 &
PID_METADATA=$!

# Step 3: Storage
echo "Starting storage..."
STORAGE_PORT=$GLOBAL_STORAGE_PORT
./build/bin/storage $STORAGE_PORT $STORAGE_PORT > tmp/distmind/log_storage.txt 2>&1 &
PID_STORAGE=$!
...
python build/bin/controller.py settings/controller.json True > tmp/distmind/log_controller.txt 2>&1 &
PID_CONTROLLER=$!
...
./build/bin/load_balancer basic 0.0.0.0 $PORT_FOR_CLIENT 0.0.0.0 $PORT_FOR_CACHE 0.0.0.0 $PORT_FOR_SERVER $CONTROLLER_IP $CONTROLLER_PORT $METADATA_IP $METADATA_PORT > tmp/distmind/log_LB.txt 2>&1 &
PID_LB=$!

sleep 5  # Give load balancer a bit of head start
...
sleep 1
FI_EFA_ENABLE_SHM_TRANSFER=0 ./build/bin/cache $LOCAL_IP $CACHE_PORT $METADATA_IP $METADATA_PORT $LOAD_BALANCER_IP $LOAD_BALANCER_PORT_CACHE cachecache $CACHE_SIZE $CACHE_BLOCK_SIZE > tmp/distmind/log_cache.txt 2>&1 &
PID_CACHE=$!

sleep 5  # Give cache a bit of head start
...
do
	...
	nohup python ./build/bin/server.py $LOCAL_IP $GPU $LOCAL_IP $CACHE_PORT $LOAD_BALANCER_IP $LOAD_BALANCER_PORT_SERVER $CONTROLLER_IP $CONTROLLER_PORT_SERVER $BACKEND $WORLD_SIZE >tmp/distmind/log_worker_$GPU.txt 2>&1 &
    PID_SERVER+=($!)
    sleep 1
done
...
# Step 12: Run client
echo "Running client training..."
./scripts/run_client_max_train.sh > tmp/distmind/log_client.txt
